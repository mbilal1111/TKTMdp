...
# ▶ Paths and logging
# ──────────────────────────────────────────────
RESULTS_ROOT = Path.cwd() / "reinforcement_learning/ppo/ppo_results_5"
...
# ▶ Hyper‑parameters 
# ──────────────────────────────────────────────
LR, GAMMA, GAE_LAMBDA, CLIP_EPS = 3e-4, 0.99, 0.95, 0.2
EPOCHS, BATCH_SIZE, ENTROPY_COEF = 4, 256, 1e-2
MAX_EPISODES, MAX_STEPS, TEST_EPS = 100, 1_000, 30
GRAD_CLIP = 5.0

# Shared-eval logging (comparable across PPO/DDPG and ontology/none)
EVAL_EVERY = 1  # set to 5 or 10 if deterministic eval each episode is too slow
...
def fanin(t: torch.Tensor):
    t.data.uniform_(-1 / math.sqrt(t.size(0)), 1 / math.sqrt(t.size(0)))
...
def _norm_training_input(x: str) -> str:
    ...
    if t in ("combined", "both", "solar_plus_heat", "solar+heat", "sum"):
        return "combined"
    raise ValueError("training_input must be one of {'solar','heat','combined'}")

class ActorCritic(nn.Module):
    def __init__(self, sd:int, ad:int):
        super().__init__()
        h1, h2 = 256, 128
        self.l1, self.l2 = nn.Linear(sd, h1), nn.Linear(h1, h2)
        self.mean, self.val = nn.Linear(h2, ad), nn.Linear(h2,1)
        self.log_std = nn.Parameter(torch.zeros(ad))
        fanin(self.l1.weight); fanin(self.l2.weight)
    def forward(self, x):
        x = F.relu(self.l1(x)); x = F.relu(self.l2(x))
        return torch.tanh(self.mean(x)), torch.exp(self.log_std), self.val(x)
...
class SolarEnv:
    ...
    def step(self, a: np.ndarray):
        ...
        a_val: float = float(np.clip(arr[0] if arr.size > 0 else 0.0, -1.0, 1.0))
        ...
        if self.ontology == "combined":
            ...
            ontology_reward = float(REWARD_MATRIX[act_idx][rel_idx]) if (self.solar is not None and self.heatpump is not None and self.baseload is not None) else 0.0
        elif self.ontology == "battery_vs_solarbaseload":
            rel_idx = self._rel_battery_vs_solarbaseload()
            ontology_reward = float(self._battery_reward_from(act_idx, rel_idx)) if (self.solar is not None and self.baseload is not None and self.battery is not None) else 0.0
        elif self.ontology == "temperature_vs_thr":
        ...
        pred_flex = (a_val + 1.0) * 0.5 * self.max_flex
...
class PPOAgent:
    ...
    def act(self, state: np.ndarray, add_noise: bool | None = None):
        s = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
        with torch.no_grad():
            mean, std, v = self.net(s)

        # Training-style call (no kwarg): return (action, logprob, value)
        if add_noise is None:
            dist   = Normal(mean, std)
            action = torch.clamp(dist.sample(), -1, 1)
            logp   = dist.log_prob(action).sum().item()
            return action.cpu().numpy()[0], logp, v.item()

        # Eval-style call (kwarg present): return only the action
        if add_noise:
            dist   = Normal(mean, std)
            action = torch.clamp(dist.sample(), -1, 1)
        else:
    ...
    def act_det(self, state: np.ndarray) -> np.ndarray:
        s = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
        with torch.no_grad():
            mean, _, _ = self.net(s)
        return torch.clamp(mean, -1, 1).cpu().numpy()[0]

    def store(self, state, action, logprob, reward, done, value) -> None:
    ...
    def _finish_path(self) -> tuple[np.ndarray, np.ndarray]:
        r = np.asarray(self.buffer.rewards, dtype=np.float32)
        d = np.asarray(self.buffer.dones,   dtype=np.float32)
        v = np.asarray(self.buffer.values + [0.0], dtype=np.float32)
    ...
    def learn(self) -> None:
        adv, ret = self._finish_path()
        S = torch.tensor(np.vstack(self.buffer.states),  dtype=torch.float32, device=device)
        A = torch.tensor(np.vstack(self.buffer.actions), dtype=torch.float32, device=device)
        L = torch.tensor(self.buffer.logprobs,           dtype=torch.float32, device=device)
        ...
        for _ in range(EPOCHS):
            ...
            for start in range(0, ds, BATCH_SIZE):
                ...
                mean, std, val = self.net(S[idx])
                dist  = Normal(mean, std)
                ratio = torch.exp(dist.log_prob(A[idx]).sum(-1) - L[idx])

                surr1 = ratio * ADV[idx]
                surr2 = torch.clamp(ratio, 1 - CLIP_EPS, 1 + CLIP_EPS) * ADV[idx]
                actor_loss  = -torch.min(surr1, surr2).mean()
                critic_loss = F.mse_loss(val.squeeze(-1), RET[idx])
...
# ▶ Utility
# ──────────────────────────────────────────────
def evaluate(agent:PPOAgent, env:SolarEnv, n:int=TEST_EPS)->Tuple[float,float]:
    rets=[]
    for _ in range(n):
        s,R=env.reset(),0
        while True:
...
# ────────────────────────────────────────────────
def evaluate_and_plot(agent: 'PPOAgent', env: SolarEnv, out_dir: str, episodes: int = 1, tag: str = "val"):
    ...
    cols = [c for c in ('reward_svh','reward_bln','reward_bat','reward_tmp') if c in df.columns]
    if cols:
        plt.figure(figsize=(10,3))
        for c in cols:
            plt.plot(df[c].values, label=c.replace('reward_',''))
...
def train_ppo(
    ...
    run_label = f"PPO_{ti}_Input__{ontology}_Ontology"
    ...
    if baseload_train_csv is None or battery_train_csv is None or temperature_train_csv is None:
        ...
        if baseload_train_csv    is None: baseload_train_csv    = bln_d
        if battery_train_csv     is None: battery_train_csv     = bat_d
        if temperature_train_csv is None: temperature_train_csv = tmp_d
    ...
    if test_dir:
        if baseload_test_csv is None or battery_test_csv is None or temperature_test_csv is None:
            ...
            if baseload_test_csv    is None: baseload_test_csv    = bln_d
            if battery_test_csv     is None: battery_test_csv     = bat_d
            if temperature_test_csv is None: temperature_test_csv = tmp_d
    ...
    if ontology in ("baseload_vs_net", "battery_vs_solarbaseload", "combined"):
    ...
    # Determine which series are required
    ...
    for ep in range(episodes):
        ...
        for _ in range(MAX_STEPS):
            ...
            agent.store(state, action, logp, reward, done, value)
        ...
        # Deterministic shared-eval snapshot (same env, reset inside rollout)
        eval_shared_return = np.nan
        eval_shared_per_step = np.nan
        eval_steps = 0
        if (ep + 1) % EVAL_EVERY == 0:
            ev = rollout_shared_metrics(env, agent.act_det, max_steps=MAX_STEPS)
        ...
        episode_metrics_rows.append({
            "episode": ep + 1,
            "train_return": float(ep_return),  # NOT comparable across ontology vs none
            ...
            "eval_shared_return": eval_shared_return,      # deterministic comparable snapshot
            "eval_shared_per_step": eval_shared_per_step,  # deterministic comparable snapshot
            "eval_steps": int(eval_steps),
        })

        if (ep + 1) % 10 == 0:
            msg = (
                f"{run_label} | Episode {ep+1:3d} | Return: {ep_return:8.3f} | "
                f"shared/step: {ep_shared/max(1,steps): .4f}"
            )
            log.info(msg)

    # Save per-episode shared metrics (used by evaluation.py)
    ...
    if test_dir:
        ...
        if test_env is not None:
            ...
            msg = f"{run_label} | TEST  μ={test_mean:.3f}  σ={test_std:.3f}"
            log.info(msg)

    # Shared policy optimality (comparable across ontology vs none AND PPO vs DDPG)
    opt_env = test_env if ("test_env" in locals() and test_env is not None) else env
    opt = rollout_optimality_metrics(opt_env, agent.act_det, max_steps=MAX_STEPS)
    ...
    if len(returns_history) >= k:
        run = np.convolve(returns_history, np.ones(k) / k, mode="valid")
    ...
    if csv_path: print("Saved per-step diagnostics to:", csv_path)
...
#
    ...
    TRAIN_DIR = r"C:\Workspace\Research\reinforcement_learning\ppo\training_data\dynamics_timeseries"
    TEST_DIR  = r"C:\Workspace\Research\reinforcement_learning\ppo\test_data\dynamics_timeseries"
